...
import { Grid, GridItem, Tooltip, VStack, useDisclosure } from '@chakra-ui/react';
...
// TODO: the online search should call the backend and not filter on the browser
// TODO: articles should be likable here
const Explore = (): JSX.Element => {
	...
	useEffect(() => {
		if (!user.data.isOffline) {
			ui.online.articles.load.liked(setOnlineArticles); // TODO: it should be other articles
				...
				createAnthology={async (name: string, description: string) =>
					user.data.isOffline
						? await ui.offline.anthologies.create({
								params: { name, description, cid: offlineArticleToAdd },
								callback: () => {
									onClose();
								},
						  })
						: await ui.online.anthologies.create({
								params: { name, description, articleId: onlineArticleToAdd },
								callback: async () => {
									onClose();
